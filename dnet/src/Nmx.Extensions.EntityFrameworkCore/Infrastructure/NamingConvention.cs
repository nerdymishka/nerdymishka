...
namespace NerdyMishka.EntityFrameworkCore.Infrastructure
{
     ...
     IIndexAddedConvention, IEntityTypeBaseTypeChangedConvention
    {
        ...
        private readonly INameTransform transformer;
            ...
            IConventionEntityTypeBuilder entityTypeBuilder, IConventionContext<IConventionEntityTypeBuilder> context)
        {
            ...
            {
                entityTypeBuilder.ToTable(this.transformer.PluralTransform(entityType.GetTableName()), entityType.GetSchema());

                if (entityType.GetViewNameConfigurationSource() == ConfigurationSource.Convention)
                {
                    entityTypeBuilder.ToView(this.transformer.PluralTransform(entityType.GetViewName()), entityType.GetViewSchema());
                }
            }
        }
            ...
            IConventionContext<IConventionPropertyBuilder> context)
        {
            ...
            if (columnName.Length > 2 && columnName.EndsWith("Id"))
            {
                columnName = columnName.Substring(0, columnName.Length - 2);
                columnName = this.transformer.SingleTransform(columnName) + "_id";

                property.SetColumnName(columnName);
            }
            else
            {
                property.SetColumnName(this.transformer.Transform(property.GetColumnBaseName()));
            }
            ...
            foreach (var storeObjectType in storeObjectTypes)
            {
                ...
                if (property.GetColumnNameConfigurationSource(identifier.Value) == ConfigurationSource.Convention)
                {
                    ...
                    property.SetColumnName(
                        this.transformer.Transform(property.GetColumnName(identifier.Value)), identifier.Value);
                }
            }
        }
                ...
                ownedEntityType.GetTableNameConfigurationSource() != ConfigurationSource.Explicit)
            {
                ...
                foreach (var keyProperty in ownedEntityType.FindPrimaryKey().Properties)
                {
                    ...
                    foreach (var storeObjectType in storeObjectTypes)
                    {
                        ...
                        if (keyProperty.GetColumnNameConfigurationSource(identifier.Value) == ConfigurationSource.Convention || this.transformer.Overwrite)
                        {
...
#pragma warning restore EF1001
                    ...
                    .Where(p => p.Builder.CanSetColumnName(null)))
                {
                    ...
                    var rewrittenPrefix = this.transformer.SingleTransform(prefix);
            ...
            IConventionAnnotation annotation, IConventionAnnotation oldAnnotation, IConventionContext<IConventionAnnotation> context)
        {
                ...
                (string)annotation.Value != ownership.PrincipalEntityType.GetTableName())
            {
                ...
                foreach (var property in entityTypeBuilder.Metadata.GetProperties()
                    ...
                    .Where(p => p.Builder.CanSetColumnName(null)))
                {
                    property.Builder.HasColumnName(this.transformer.Transform(property.GetDefaultColumnBaseName()));
                }
                ...
                {
                    key.Builder.HasName(this.transformer.Transform(key.GetDefaultName()));
                }
            }
        }
        ...
        public void ProcessForeignKeyAdded(IConventionForeignKeyBuilder relationshipBuilder, IConventionContext<IConventionForeignKeyBuilder> context)
            => relationshipBuilder.HasConstraintName(this.transformer.Transform(relationshipBuilder.Metadata.GetConstraintName()));

        public void ProcessKeyAdded(IConventionKeyBuilder keyBuilder, IConventionContext<IConventionKeyBuilder> context)
        {
            var name = keyBuilder.Metadata.GetName();
            keyBuilder.HasName(this.transformer.Transform(name));
        }

        public void ProcessIndexAdded(
            IConventionIndexBuilder indexBuilder,
            IConventionContext<IConventionIndexBuilder> context)
            => indexBuilder.HasDatabaseName(this.transformer.Transform(indexBuilder.Metadata.GetDatabaseName()));
    }
}
