...
namespace NerdyMishka.Security.Cryptography
{
    ...
    public class Blake2b : HashAlgorithm
    {
        ...
        private static readonly int[] Sigma = new int[Rounds * 16]
        {
        ...
        public byte[] Key
        {
            ...
            set
            {
                ...
                if (this.key.SequenceEqual(value))
                    return;
        ...
        {
            get => this.personalization?.AsSpan().ToArray();

            set
            {
                if (this.personalization == null && value == null)
                    return;
                ...
                if (value.Length != 16)
                    throw new ArgumentOutOfRangeException(
                        nameof(this.Personalization),
                        "Personalization must have a length of 16");

                if (this.personalization is not null && this.personalization.SequenceEqual(value))
                    return;
        ...
        private void G(int a, int b, int c, int d, int r, int i)
        {
            ...
            v[a] += v[b] + m[p0];
            v[d] = BitShift.RotateRight64(v[d] ^ v[a], 32);
            v[c] += v[d];
            v[b] = BitShift.RotateRight64(v[b] ^ v[c], 24);
            v[a] += v[b] + m[p1];
            v[d] = BitShift.RotateRight64(v[d] ^ v[a], 16);
            v[c] += v[d];
            v[b] = BitShift.RotateLeft64(v[b] ^ v[c], 63);
        }
