...
Get-WindowsUpdate = Windows Update / Auto Update
...
Get-Content a username, such as User01, Domain01\User01, or User@Contoso.com. Or, enter a PSCredential object,
...
.EXAMPLE
Get-Process C:\> Test-PendingReboot
...
.EXAMPLE
Get-Process C:\> (Test-PendingReboot).IsRebootPending
True

This example will return a bool value based on the pending reboot test for the local computer.

.EXAMPLE
Get-Process C:\> Test-PendingReboot -ComputerName DC01 -Detailed
...
.EXAMPLE
Get-Process C:\> Test-PendingReboot -ComputerName DC01 -SkipConfigurationManagerClientCheck -SkipPendingFileRenameOperationsCheck -Detailed
    ...
    process
    {
        ...
        foreach ($computer in $ComputerName)
        {
            try
            {
                ...
                $registryComponentBasedServicing = (Invoke-CimMethod @invokeWmiMethodParameters).sNames -contains 'RebootPending'

                ## Query WUAU from the registry
                $invokeWmiMethodParameters.ArgumentList = @($hklm, 'SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\')
                $registryWindowsUpdateAutoUpdate = (Invoke-CimMethod @invokeWmiMethodParameters).sNames -contains 'RebootRequired'

                ## Query JoinDomain key from the registry - These keys are present if pending a reboot from a domain join operation
                $invokeWmiMethodParameters.ArgumentList = @($hklm, 'SYSTEM\CurrentControlSet\Services\Netlogon')
                $registryNetlogon = (Invoke-CimMethod @invokeWmiMethodParameters).sNames
                ...
                $registryActiveComputerName = Invoke-CimMethod @invokeWmiMethodParameters

                $invokeWmiMethodParameters.ArgumentList = @($hklm, 'SYSTEM\CurrentControlSet\Control\ComputerName\ComputerName\', 'ComputerName')
                $registryComputerName = Invoke-CimMethod @invokeWmiMethodParameters
                ...
                if (-not $PSBoundParameters.ContainsKey('SkipConfigurationManagerClientCheck'))
                {
                    ...
                    try
                    {
                        $sccmClientSDK = Invoke-CimMethod @invokeWmiMethodParameters
